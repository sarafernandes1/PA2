Shader "Hidden/DitheringShader"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _Dithering ("Dither", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float uvSphereProject(sampler2D tex, float2 texel, float3 dir)
            {
                ...
                return tex2D(tex, texel * _Tiling * float2(u, v)).r;
            }
            ...
            float edge(float2 uv, float2 delta)
            {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float ditherLum = (col.a < 0.5) ? cubeProject(_Noise, _Noise_TexelSize.xy, dir) : cubeProject(_Dithering, _Dithering_TexelSize.xy, dir);

                float2 edgeData = edge(i.uv.xy, _MainTex_TexelSize.xy * 2.0f);

                lum = (edgeData.y < _Threshold) ? lum : ((edgeData.x < 0.1f) ? 1.0f : 0.9f);
